...
class Namespace(object):
    ...
    def accesspoint_create(self, name, path):
        data = {'path': path}
        self.api_call("PUT", '/' + name.strip('/'), data=json.dumps(data))

    def accesspoint_delete(self, name):
        self.api_call("DELETE", '/' + name.strip('/'))
    ...
    def acl_set(self, path, acls, nsaccess=False):
        ...
        self.api_call("PUT", path + options, data=json.dumps(acls))
    ...
    def dir_create(self, path, recursive=True):
        ...
        self.api_call("PUT", path, params=options, headers=headers)

    def dir_delete(self, path):
        '''delete a directory'''
        options = ""
        self.api_call("DELETE", path + options)
